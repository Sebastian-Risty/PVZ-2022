...
static LRESULT CALLBACK MainTimerLoop(HWND hwnd, UINT uMsg, int32_t timerId, DWORD dwTime)
{
	...
	// window clamping
	WindowRect gameWindowRect = GetGameRect(hwnd);
	ImVec2 winClamp = ImVec2(gameWindowRect.m_WindowWidth, gameWindowRect.m_WindowHeight);
...
void Menu::InitilizeWindow(HMODULE hInstance, const char* windowClassName, const char* windowName) noexcept
{
	...
	this->m_D3D9PresentParams.PresentationInterval = D3DPRESENT_INTERVAL_IMMEDIATE;
	//this->m_D3D9PresentParams.FullScreen_RefreshRateInHz = 144;
...
{
	if (ImGui::Begin("DinkumWare.com", &this->m_MenuOpened, ImGuiWindowFlags_NoTitleBar))
	{
		ImGui::SetWindowSize(ImVec2(640, 480), ImGuiCond_Once);

		// clamp menu within window
		WindowRect gameWindowRect = GetGameRect(this->m_GameWindowHandle);
		ImVec2 clamps = ImVec2(gameWindowRect.m_WindowWidth - static_cast<int32_t>(ImGui::GetWindowSize().x), gameWindowRect.m_WindowHeight - static_cast<int32_t>(ImGui::GetWindowSize().y));
		
		float winX = ImGui::GetWindowPos().x, winY = ImGui::GetWindowPos().y;
		if (winX > clamps.x) {
			ImGui::SetWindowPos(ImVec2(clamps.x, winY));
			ImGui::StartMouseMovingWindow(ImGui::GetCurrentWindow());
		}
		if (winX < 0) {
			ImGui::SetWindowPos(ImVec2(0, winY));
			ImGui::StartMouseMovingWindow(ImGui::GetCurrentWindow());
		}
		if (winY > clamps.y) {
			ImGui::SetWindowPos(ImVec2(winX, clamps.y));
			ImGui::StartMouseMovingWindow(ImGui::GetCurrentWindow());
		}
		if (winY < 0) {
			ImGui::SetWindowPos(ImVec2(winX, 0));
			ImGui::StartMouseMovingWindow(ImGui::GetCurrentWindow());
		}

		char debug[256];
		sprintf_s(debug, "Debug: (%i,%i)", 5, 5);
		//ImGui::Text(debug);

		// calculate FPS
		char fps[256];
		sprintf_s(fps, "FPS: %i", static_cast<int>(10.f / ImGui::GetIO().DeltaTime));
		ImGui::Text(fps);

		// window position
		ImVec2 position = ImGui::GetWindowPos();
		char pos[256];
		sprintf_s(pos, "Window Pos: (%i,%i)", static_cast<int>(position.x), static_cast<int>(position.y));
		ImGui::Text(pos);
